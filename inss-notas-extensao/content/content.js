...
function deleteNoteHandler(container, protocolo) {
  ...
  deleteNote(protocolo).then(deleted => {
    ...
    if (deleted) {
      ...
      // Encontrar o innerDiv para recriar o botão
      const tdInteressado = container.closest('td');
      if (tdInteressado) {
        const innerDiv = tdInteressado.querySelector('div');
        if (innerDiv) {
          // Remover completamente o container antigo
          container.remove();

          // Criar novo botão de adicionar
...
function processRow(tr) {
  ...
  innerDiv.appendChild(noteContainer);
}
...
async function init() {
  console.log('[NotasPat] Inicializando v1.1.0...');
